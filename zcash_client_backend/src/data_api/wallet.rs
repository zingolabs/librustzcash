    ...
    FeeRuleT: FeeRule,
{
    ...
    //
    // This must come after every value-bearing transparent output has been added: the null-data
    // output has no `transparent_output_meta` entry, so adding it earlier would shift the vout
    // indices that entry `n` of that vector is assumed to match.
